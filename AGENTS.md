...
搜索业务的问题形式是**约束求解，不是自由优化**。所有 E1–E7 的结果最终都要回答这一个问题：

> **给定 P99 ≤ 150 ms 的延迟预算，最多能过几级、每级保留几个候选、用什么精度，才能让 MAP@25 最高？**

因此报告里每组实验除了给「优化提升了多少」，还必须给「它把可行域推开了多少」。
三个业务口径的最终指标：

| 口径 | 定义 | 为什么业务方只看这个 |
|---|---|---|
| **P99 延迟** | 端到端 p99（ms） | 超预算就是不可上线，均值没有意义 |
| **单卡 QPS 上限** | latency–throughput 曲线的 knee point | 决定要几张卡 |
| **GPU-秒 / 千次查询** | 成本折算 | 决定这个方案划不划算 |

### 项目边界（主动收缩，不要越界）

**本项目是「LLM 排序层的推理优化」，不是「完整搜索引擎」。**

- 候选库仅 4791 条，暴力矩阵乘即可，**不做 ANN / 倒排 / 多路召回**——那是召回层工程，与 LLM 推理优化正交
- 不做在线学习、不做特征工程、不做多目标融合
- 被问到时正面回答：LLM reranker 是搜索/推荐当前正在攻的方向，其落地瓶颈就是延迟与成本，本项目正是解决这一段
...
| **容量** | **单卡 QPS 上限**（latency–throughput 曲线的 knee point）|
